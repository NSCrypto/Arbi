...
public class CachingOrderBook {
	...
	private LoadingCache<CurrencyPair, List<LimitOrder>> buyOrders = CacheBuilder.newBuilder().concurrencyLevel(4).expireAfterWrite(5, TimeUnit.SECONDS)
	...
	private LoadingCache<CurrencyPair, List<LimitOrder>> sellOrders = CacheBuilder.newBuilder().concurrencyLevel(4).expireAfterWrite(5, TimeUnit.SECONDS)
			.build(new CacheLoader<CurrencyPair, List<LimitOrder>>() {
				@Override
				public List<LimitOrder> load(CurrencyPair key) throws Exception {
					try {
