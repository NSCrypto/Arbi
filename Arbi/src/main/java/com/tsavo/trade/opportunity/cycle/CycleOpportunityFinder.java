...
public class CycleOpportunityFinder implements OpportunityFinder {
	...
	public CycleOpportunityFinder(List<Exchange> someExchanges)
			throws IOException {
		cryptsy = new Cryptsy();
		...
		cycles = CycleFinder
				.findCurrencyCycle("BTC", Arrays.asList(new CurrencyPair("BTC",
						"USD"), new CurrencyPair("LTC", "BTC"),
						new CurrencyPair("LTC", "USD"), new CurrencyPair("XRP",
								"USD"), new CurrencyPair("XRP", "BTC"),
						new CurrencyPair("DRK", "USD"), new CurrencyPair("DRK",
								"BTC"), new CurrencyPair("DRK", "LTC")));
		usdcycles = CycleFinder
				.findCurrencyCycle("USD", Arrays.asList(new CurrencyPair("BTC",
						"USD"), new CurrencyPair("LTC", "BTC"),
						new CurrencyPair("LTC", "USD"), new CurrencyPair("XRP",
								"USD"), new CurrencyPair("XRP", "BTC"),
						new CurrencyPair("DRK", "USD"), new CurrencyPair("DRK",
	...
	public List<Opportunity> findOpportunities() throws ExchangeException,
		...
		List<CurrencyCycle> viableCycles = cycles.GetAllLeaves().stream()
				.filter(x -> x.balance != null && x.balance.floatValue() > 1)
				.collect(Collectors.<CurrencyCycle> toList());
		viableCycles.addAll(usdcycles.GetAllLeaves().stream()
				.filter(x -> x.balance != null && x.balance.floatValue() > 1)
				.collect(Collectors.<CurrencyCycle> toList()));
		...
		return viableCycles.stream().map(x -> new CycleOpportunity(x))
				.collect(Collectors.<Opportunity> toList());
